...
function init(iddiv) {
  ...
  for (var i = 0; i < numalpha; i++) {
    ...
    for (var j = 0; j < numlets; j++) g_letpool.push(whichlt);
  }
...
function checkValidPlacement(placement) {
  if (placement.length === 0) return { played: '', msg: t('no letters were placed.') };
  ...
  if (dx == 1 && dy == 1) return { played: '', msg: t('word must be horizontal or vertical.') };

  if (g_board_empty && !onStar) return { played: '', msg: t('first word must be on the star.') };
  ...
  if (dx === 0 && dy === 0) {
    ...
    if (
      (minx > 0 && g_board[minx - 1][miny] !== '') ||
      (minx < mbx - 1 && g_board[minx + 1][miny] !== '')
    )
      dx = 1;
    else if (
      (miny > 0 && g_board[minx][miny - 1] !== '') ||
...
function find_best_move(opponent_rack) {
  ...
  letters = [];
  for (var i = 0; i < num; i++) letters[i] = opponent_rack.charAt(i);
...
//   var odeduct = 0;
//   for (var i = 0; i < oleft.length; i++) odeduct += g_letscore[oleft.charAt(i)];

//   var pdeduct = 0;
//   for (i = 0; i < pleft.length; i++) pdeduct += g_letscore[pleft.charAt(i)];
...
//   html += "<font size='+2'>" + msg + '</font>';
//   g_bui.prompt(html, '');
//   console.log(g_pscore + 'hellowinner score');
//   console.log(g_oscore + 'winner score');
//   // Prepare data to send to the backend
//   const userId = localStorage.getItem('userId');
//   const token = localStorage.getItem('authToken');
//   var data = {
//     userId,
//     playerScore: g_pscore,
//     computerScore: g_oscore,
//     result: msg,
//     completedTime: new Date().toISOString(), // Add completed time in ISO format
//   };
//   console.log('Winner data:', data);
//   // Send data to the backend
//   fetch('http://localhost:9000/apis/games/scrabble', {
//     method: 'POST',
//     headers: {
...
//     .then((response) => {
//       if (!response.ok) {
//         throw new Error('Failed to send winner data to the backend');
//       }
...
function onPlayerMove() {
  ...
  } else play_word = find_best_move(ostr);
  ...
  if (play_word !== null) {
    ...
    g_bui.hideBusy();
    g_passes++; // increase consecutive opponent passes
    if (g_passes >= g_maxpasses) announceWinner();
    else {
...
function getBestScore(regex, letters, ax, ay) {
  ...
  for (var wlc = regex.min - 2; wlc < regex.max - 1; wlc++) {
    ...
    if (id in g_matches_cache) matches = g_matches_cache[id];
    else {
      matches = [];
...
function getOrthWordScore(lseq, lscr, x, y, dx, dy) {
  ...
  if (!(orthword in g_wordmap)) return { score: -1, word: orthword };
...
function getRegex(dir, ax, ay, rack) {
  ...
  if (blanks > 0) {
    ...
    if (p == max)
      ...
      if (g_board[x][y] === '') regex += '{' + minplay + ',' + blanks + '}';
      else {
        regex += '{' + blanks + '}';
        for (i = p + 1; i < max; i++) {
          l = g_board[x][y];
